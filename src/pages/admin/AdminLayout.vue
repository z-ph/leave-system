...
import { PAGE_TITLES } from "@/router/constants";
  ...
  <el-container :style="{ minHeight: '100vh' }">
    <el-aside v-if="!isMobile" :style="{ width: '15%', minHeight: '100vh' }">
      <el-menu :default-active="active" router :style="{ height: '100%' }">
        <el-menu-item index="/admin/approvals">{{ PAGE_TITLES.APPROVALS }}</el-menu-item>
        <el-menu-item index="/admin/requests">{{ PAGE_TITLES.REQUESTS }}</el-menu-item>
        <el-menu-item index="/" :style="{color:'red'}">返回用户首页</el-menu-item>
        <el-menu-item
          index="/admin/admins"
          v-if="hasRole(currentUserRole, [Role.ADMIN])"
          >{{ PAGE_TITLES.ADMINS }}</el-menu-item
        >
      </el-menu>
    </el-aside>
    <el-container>
